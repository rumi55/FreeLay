...
if($_REQUEST['action'] == 'incoming') {
	...
	if(preg_match('/^\+[0-9]{10,14}$/', $gateway) || preg_match('/^\*([0-9#])/', $gateway) ) {
...
if($_REQUEST['action'] == 'outgoing') {
	...
	$s = "SELECT * FROM sms WHERE agent_phone LIKE '$gateway%' AND status='0' AND delivery_date < NOW() ORDER BY modified_date ASC LIMIT 500";
	...
	if(mysqli_num_rows($q) >= 1) {
		...
		while($get = mysqli_fetch_array($q, MYSQLI_ASSOC) ) {
			...
			$u = "UPDATE sms SET status='2' WHERE msg_id='$id'";
	...
	exit();
		...
		$message = "No queued messaged in the DB:SMS as at ".date('D d M g:ia').' for '.$gateway;
